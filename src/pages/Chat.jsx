...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setMessages([
        { role: 'ai', content: 'Hi, I am Abhi. What insights are you looking for today?' }
      ]);
    }
  }, []);
  ...
  const clearChat = () => {
    const defaultMsg = [{ role: 'ai', content: 'Chat cleared. How can I help you now?' }];
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'ai', content: 'Sorry, I encountered an error connecting to my brain. Please try again later.' }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex flex-col h-[100dvh] bg-[#FAFAFB] text-gray-800 font-['Inter'] relative w-full overflow-hidden">
      ...
      <header className="flex-none px-4 py-3 sm:px-6 sm:py-4 flex items-center justify-between border-b border-gray-100 bg-white/70 backdrop-blur-xl z-20 shadow-sm sticky top-0 md:bg-white/50">
        ...
        <div className="font-semibold tracking-wide text-gray-800 flex items-center gap-2">
          Abhi <span className="flex h-2 w-2 rounded-full bg-[#6D5DFC] opacity-80"></span>
        </div>
        <button
          onClick={clearChat}
      ...
      {/* Main Chat Area */}
      <main className="flex-1 overflow-y-auto px-4 py-6 sm:py-8 z-10 w-full scrollbar-hide scroll-smooth">
